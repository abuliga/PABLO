...
class DatasetConfs:
    ...
    def __init__(self, dataset_name, where_is_the_file='',data=False):
        if dataset_name in ["traffic_fines_%s" % formula for formula in range(1, 3)]:
            ...
            self.timestamp_col = {dataset: "CompleteTimestamp"}
            ...
            self.dynamic_num_cols = {dataset: ["expense", "timesincelastevent", "timesincecasestart", "timesincemidnight", "eventnr",
                                         "month", "weekday", "hour", "opencases"]}
            self.static_num_cols = {dataset: ["amount", "points"]}
        elif dataset_name in ["sepsis_cases_1","sepsis_cases_2","sepsis_cases_3","sepsis_cases_4","sepsis_cases_5","sepsis",'sepsis_cases_relabelled_dtime_ERTriag_LacticAcid']:
            ...
            self.dynamic_num_cols = {dataset: ["timesincelastevent","Leucocytes","CRP","LacticAcid"  ,"month", "weekday", "hour" , "timesincecasestart", "eventnr", "opencases" , "timesincemidnight"]}
            ...
            # features for classifier
            self.static_cat_cols = {dataset: ["PartDesc", "Rework"]}
            self.static_num_cols = {dataset: ["WorkOrderQty"]}
            self.dynamic_cat_cols = {dataset: ["Resource", "ReportType"]}
            self.dynamic_num_cols = {
                dataset: ["QtyCompleted", "QtyforMRB", "activityduration", "hour", "weekday", "month",
                          "timesincemidnight", "timesincelastevent", "timesincecasestart", "eventnr", "opencases"]}
            ...
            # features for classifier
            self.dynamic_cat_cols = {dataset: ['Resource', 'Action', 'EventOrigin', 'lifecycle:transition',
                                         "Accepted", "Selected"]}
            ...
            self.dynamic_num_cols = {dataset: ['FirstWithdrawalAmount', 'MonthlyCost', 'NumberOfTerms', 'OfferedAmount'
                                         ...
                                         "eventnr", "month", "weekday", "hour", "opencases"]}
            self.static_num_cols = {dataset: ['RequestedAmount','CreditScore']}
            ...
            self.dynamic_num_cols = {dataset: ["hour", "weekday", "month", "timesincemidnight", "timesincelastevent",
                                         "timesincecasestart", "eventnr", "opencases"]}
            self.static_num_cols = {dataset: ['AMOUNTREQ']}
            ...
            self.case_id_col = {dataset: "Case ID"}
            self.activity_col = {dataset: "Activitycode"}
            self.resource_col = {dataset: "Resource"}
            self.timestamp_col = {dataset: "time:timestamp"}
            self.label_col = {dataset: "label"}
            self.pos_label = {dataset: "deviant"}
            ...
            # features for classifier
            self.dynamic_cat_cols = {dataset: ["Resource", "Section", "Specialismcode1", "group"]}
            self.static_cat_cols = {dataset: ["Diagnosis", "Treatmentcode", "Diagnosiscode", "Specialismcode",
                                        "DiagnosisTreatmentCombinationID"]}
            self.dynamic_num_cols = {dataset: ["Numberofexecutions", "hour", "weekday", "month", "timesincemidnight",
                                         "timesincelastevent", "timesincecasestart", "eventnr", "opencases"]}
            self.static_num_cols = {dataset: ["Age"]}
