...
def dice_impressed(CONF, predictive_model, cf_df, encoder, query_instance, method, support, optimization,
                   timestamp_col_name, model_path, neighborhood_size, random_seed=None, diversity_weight=None,
                   sparsity_weight=None, proximity_weight=None,
...
def continuous_distance(query_instance, cf_list, cont_feature_index, metric='euclidean', X=None, agg=None):
    if metric == 'mad':
        ...
        dist = cdist(np.array(query_instance, dtype='float')[:, cont_feature_index], cf_list[:, cont_feature_index],
...
def plausibility(query_instance, predictive_model, cf_list, nr_of_cfs,
    ...
    for cf in cf_list:
        ...
        neigh_dist = distance_mh(np.array(query_instance).astype('int'), X_y.to_numpy().astype('int'),
                                 continuous_features,
...
def conformance_score(CONF, encoder, df, dataset, features_names, d4py, query_instance, model_path, timestamp_col_name):
    ...
    conformance_score = [len(v) / len(query_patterns) if len(query_patterns) > 0 else 0.0 for v in
                         model_check_res.values()]
...
def model_discovery(CONF, encoder, df, dataset, features_names, d4py, model_path, support, timestamp_col_name):
    ...
    discovered = d4py.filter_discovery(min_support=support,
                                       output_path=model_path + dataset + '_' + str(CONF['prefix_length']) + '.decl')


columns = ['dataset', 'model', 'method', 'optimization', 'prefix_length', 'idx', 'desired_nr_of_cfs', 'generated_cfs',
