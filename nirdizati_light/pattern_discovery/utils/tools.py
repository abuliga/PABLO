...
def check_consecutive_edges(graph):
...
def Pattern_Extender(All_extended_patterns_2, EventLog_graphs, data, case_id, activity,
                     Max_gap_between_events=10, Direct_predecessor=False, Direct_successor=True, Eventual_following=True,
                     Eventually_preceding=False):
    ...
    for chosen_pattern_ID in All_extended_patterns_2:
        ...
        for idx, case in enumerate(All_extended_patterns_2[chosen_pattern_ID]['Instances']['case']):
            ...
            if Eventually_preceding and len(in_pattern_nodes) > 0:
            ## all non-direct nodes
                Eventual_relations_nodes = set(embedded_trace_graph.nodes).difference(
                    in_pattern_nodes.union(out_pattern_nodes))
                Eventual_relations_nodes.remove('pattern')
    ...
    return Extension_3_patterns, Extended_patterns_at_stage
